...
## The product name is not decided — keep it that way

The brand lives in exactly one place: `src/config/product.ts`. Everything else
imports `PRODUCT.name`. This is deliberate, so renaming stays a one-file edit.
